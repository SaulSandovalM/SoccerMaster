...
class TableContainer4 extends Component {
  state = {
    data:[],
    loading:true,
    openForm:false,
  ...
  componentWillMount(){
    firebase.database().ref('torneocuatro').on('child_added',s=>{
    ...
    firebase.database().ref('torneocuatro').on('child_removed',s=>{
      const {data} = this.state;
      let item = s.val();
      item["key"] = s.key;
  ...
  saveItem = () => {
    ...
    this.closeForm();
    firebase.database().ref('torneocuatro')
      .push(newItem)
    ...
    return(
      <div>
        ...
        <div className='torneo'>
          <h2>Torneo Copa Libertadores</h2>
...
export default TableContainer4;
