...
class EquipoContainer extends Component{
  ...
  componentWillMount(){
    firebase.database().ref('/torneocuatro/equipos/teamlibertadores/').on('child_added',s=>{
    ...
    firebase.database().ref('/torneocuatro/equipos/teamlibertadores/').on('child_removed',s=>{
      const {data} = this.state;
      let item = s.val();
      item["key"] = s.key;
  ...
  saveItem = () => {
    ...
    this.closeForm();
    firebase.database().ref('/torneotres/equipos/teamliber/libertadores')
      .push(newItem)
