...
class EquipoContainer extends Component{
  ...
  componentWillMount(){
    firebase.database().ref('/CopaLibertadores/equipos/teamlibertadores/').on('child_added',s=>{
    ...
    firebase.database().ref('/CopaLibertadores/equipos/teamlibertadores/').on('child_removed',s=>{
      const {data} = this.state;
      let item = s.val();
      item["key"] = s.key;
  ...
  saveItem = () => {
    ...
    this.closeForm();
    firebase.database().ref('/CopaLibertadores/equipos/teamliber/libertadores')
      .push(newItem)
  ...
  render(){
    ...
    return(
      <div className='torneo'>
        <h2>Torneo Copa de Libertadores</h2>
