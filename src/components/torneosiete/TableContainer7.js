...
class TableContainer7 extends Component{
  state = {
    data:[],
    loading:true,
    openForm:false,
  ...
  componentWillMount(){
    firebase.database().ref('torneosiete').on('child_added',s=>{
    ...
    firebase.database().ref('torneosiete').on('child_removed',s=>{
      const {data} = this.state;
      let item = s.val();
      item["key"] = s.key;
  ...
  saveItem = () => {
    ...
    this.closeForm();
    firebase.database().ref('torneosiete')
      .push(newItem)
...
export default TableContainer7;
